...
void drawPlayers() {
  ...
  //  players[i].draw(pg);
    ...
    //pgIntersection.background(0.0f, 1.0f);
  //for (int i=0; i<players.length; i++) {
    //pgIntersection.image(players[i].getPG(), 0, 0);
    //pgIntersection.blend(players[i].getPG(),0, 0, width, height, 0, 0, width, height, ADD);
...
//  //    pgIntersection.endDraw();
  ...
  pgIntersection.beginDraw();
  pgIntersection.background(0.0f, 0.0f);
  for (int i=0; i<players.length; i++) {
    PImage n = players[i].getPG().get();
    //pgIntersection.image(n, 0, 0);
    pgIntersection.blend(n, 0, 0, width, height, 0, 0, width, height, ADD);
  }
  pgIntersection.endDraw();
