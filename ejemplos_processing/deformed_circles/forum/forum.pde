...
void draw() {
  if (savePDF) {
    timestamp = timestamp();
    PGraphicsPDF pdf = (PGraphicsPDF) createGraphics(width, height, PDF, timestamp + ".pdf");
    beginRecord(pdf);
  }
...
void generateRadiusPoints() {
  ...
  for (int i=0; i<numberOfCircles; i++) {
    for (int j=0; j<numberOfPoints; j++) {
      radiusPoint[i][j] = int(outerRing-controlValueArray[j]*spaceBetween*i);
    }
  }
}